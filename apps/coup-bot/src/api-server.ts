...
export function startApiServer(sessionManager?: SessionManager): void {
  ...
  const server = createServer(async (req, res) => {
    ...
    if (req.method === "GET" && req.url?.startsWith("/api/game/")) {
      try {
        ...
        json(res, 200, { gameId: session.gameId });
      } catch (err) {
    ...
    if (req.method === "POST" && req.url === "/api/token") {
      try {
        console.log("[api] POST /api/token");
        const body = JSON.parse(await readBody(req)) as { code?: string };
        if (!body.code) {
          console.log("[api] missing code");
        ...
        console.log("[api] exchanging code for token");
        ...
        console.log("[api] discord response:", { status: upstream.status, hasToken: !!data.access_token, error: data.error });
        ...
        console.error("[api] token endpoint error:", err);
        json(res, 500, { error: String(err) });
      }
      return;
    }
