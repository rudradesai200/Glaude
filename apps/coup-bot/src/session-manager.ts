...
export class SessionManager {
  ...
  recover(): void {
    ...
    for (const recovered of active) {
      ...
      if (recovered.phase === "LOBBY") {
        ...
        const deserializedState = def.deserializeState(row.state);
        // Ensure moveNumber exists for compatibility
        const stateWithMoveNumber = {
          ...deserializedState,
          moveNumber: (deserializedState as { moveNumber?: number }).moveNumber ?? 0,
        };
  ...
  makeMove(
    ...
    const currentMoveNumber = (state as { moveNumber?: number }).moveNumber ?? 0;
    const newStateWithMoveNumber = {
      ...newState,
    ...
    if (outcome) {
      ...
      this.#sessions.set(channelId, ended);
      const updatedPlaying: PlayingSession = { ...session, state: newStateWithMoveNumber };
      persistMoveAndState(this.#db, updatedPlaying, playerId, serializedMove, currentMoveNumber);
      persistEnded(this.#db, ended);
      return ok({ session: ended, renderCtx: definition.buildRenderContext(newStateWithMoveNumber, seats) });
    }

    const updated: PlayingSession = { ...session, state: newStateWithMoveNumber };
    this.#sessions.set(channelId, updated);
    persistMoveAndState(this.#db, updated, playerId, serializedMove, currentMoveNumber);
    return ok({ session: updated, renderCtx: definition.buildRenderContext(newStateWithMoveNumber, seats) });
  }
